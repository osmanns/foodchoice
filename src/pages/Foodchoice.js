...
import Foodchoice1 from "./Foodchoice1"
import Foodchoice2 from "./Foodchoice2"
import Foodchoice3 from "./Foodchoice3"
...
function Foodchoice() {
    ...
    const getItems = async () => {
        ...
        const response = await Axios.get('http://159.65.133.73/api/stage', {
            ...
            console.log(response.data.stage_1)
            var datacount = response.data.stage_1.length
            setDataCount(datacount)
            setRecords(response.data.stage_1.reverse())
        }).catch((error) => {
        ...
        <>
            ...
            <AppBar position="static">
                <Tabs value={value} onChange={handleChange} aria-label="simple tabs example">
                    <Tab label="ข้อมูลรอดำเนินการ" {...a11yProps(0)} />
                    <Tab label="ข้อมูลอยู่ระหว่างดำเนินการ" {...a11yProps(1)} />
                    <Tab label="ข้อมูลรอการยืนยัน" {...a11yProps(2)} />
                    <Tab label="ข้อมูลดำเนินการเสร็จสิ้น" {...a11yProps(3)} />
                </Tabs>
            </AppBar>
            <TabPanel value={value} index={0}>
                <Toolbar>
                    ...
                    />
                    <span className="box-data-count">ข้อมูลรอดำเนินการ จำนวนทั้งหมดในระบบ : {dataCount} รายการ</span>
                </Toolbar>
                            ...
                            recordsAfterPagingAndSorting().map((items, index) => (
                                <TableRow key={items.products_per_serving_id}>
                                    ...
                                    <TableCell style={{ width: 10 }} align="right">
                                        <Link to={`/foodchoiceAdd/${items.products_per_serving_id}`} className="link-edit" ><EditIcon /></Link>
            ...
            {selectedTab === 1 && <Foodchoice1 />}
            {selectedTab === 2 && <Foodchoice2 />}
            {selectedTab === 3 && <Foodchoice3 />}
            </Paper>
        </>
    )
}
